...
// The Player can choice Number Of Questions - Level - Operations
short ReadNumber(string Message)
{
    ...
    while (true)
    {
        ...
        if (cin.fail()) {
            // المستخدم دخل حاجة مش رقم
            cin.clear(); // نعيد حالة الـcin من الفشل
            cin.ignore(numeric_limits<streamsize>::max(), '\n'); // نتجاهل الإدخال الغلط
            cout << "Error: please enter a valid number.\n";
        }
        else
        {
            cin.ignore(numeric_limits<streamsize>::max(), '\n'); // نتأكد إن أي باقي إدخال اتجاهل
            return Number;
        }
    }
}
...
void PlayMathGame()
{
    ...
    Quizz.NumberOfQuestions = ReadNumber("How Many Questions ?");

    Quizz.QuestionsLevel = (enQuestionsLevel)ReadNumber("What is Level Do You Want \nEasyLevel = 1, MedLevel = 2, HardLevel = 3, Mix = 4 ?");

    Quizz.OpType = (enOperationType)ReadNumber("What is Type Of Questions ?\n Add = 1 , Sub = 2 , Multi = 3 , Dvi = 4 , Mix = 5");
